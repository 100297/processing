Button fireButton = new Button(18,100,100,"FIRE");
...
PImage scare;
void setup() {
  scare = loadImage("search.jpg");
  farm = loadImage("images.jpg");
  size(600,470);
  image(farm,0,0);
    pumkin = loadImage("imgres.jpg");
}

void draw() {
  image(scare,crowX,crowY);
  image(pumkin,pumkinX,pumkinY);
  if(mode == FIRE) {
  background(farm);
  image(scare,crowX,crowY);
  pumkinX += vx;
